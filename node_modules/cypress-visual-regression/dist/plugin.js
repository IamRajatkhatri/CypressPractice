...
var import_sanitize_filename = __toESM(require("sanitize-filename"), 1);
...
var updateSnapshot = async (options) => {
  ...
  try {
    ...
    logger.error(`Failed to create directory '${destDir}' with error:`, error2);
  ...
  try {
    ...
    logger.error(`Failed to copy file '${destDir}' with error:`, error2);
...
var moveActualSnapshotIfNeeded = (actualScreenshot, relativeToCommonRoot) => {
  if (relativeToCommonRoot != null) {
    ...
    try {
      if (!(0, import_fs2.existsSync)(newDirectory)) {
        (0, import_fs2.mkdirSync)(newDirectory, { recursive: true });
      }
      (0, import_fs2.renameSync)(actualScreenshot, newPath);
      pruneEmptyDirectoriesInverse(path.dirname(actualScreenshot));
    } catch (error2) {
      logger.error(`Failed to move file '${actualScreenshot}' with error:`, error2);
...
var pruneEmptyDirectoriesInverse = (directory) => {
  if ((0, import_fs2.readdirSync)(directory).length === 0) {
    (0, import_fs2.rmdirSync)(directory);
...
async function generateImage(diffPNG, imagePath) {
  ...
  try {
    ...
    logger.error(`Failed to create directory '${dirName}' with error:`, error2);
  ...
  return await new Promise((resolve, reject) => {
    ...
    file.on("error", (error2) => {
      logger.error(`Failed to write stream '${imagePath}' with error:`, error2);
      reject(new Error(`cannot create file '${imagePath}'.`));
    });
    diffPNG.pack().pipe(file).on("finish", () => {
      resolve(true);
    }).on("error", (error2) => {
      logger.error(`Failed to parse image '${imagePath}' with error:`, error2);
      reject(error2);
    });
  });
}
